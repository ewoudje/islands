...
object IslandWorldPresets {
    private val PRESETS = DeferredRegister.create(IslandMod.MOD_ID, SpecialClientRegistries.WORLD_PRESETS)
    private val DIMENSION_EFFECTS = DeferredRegister.create(IslandMod.MOD_ID, SpecialClientRegistries.DIMENSION_SPECIAL_EFFECTS)
    ...
    val VOID_ISLANDS = preset("void_islands") { createWorldPreset("void_islands".id(), { registry, seed ->
    ...
    private fun dimensionType(name: String): ResourceKey<DimensionType> =
        ResourceKey.create(Registry.DIMENSION_TYPE_REGISTRY, name.id())
